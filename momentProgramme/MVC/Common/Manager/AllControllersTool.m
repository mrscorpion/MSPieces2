...
#import "DrawerViewController.h" // 左侧的[本项目原来的]
...
@property (nonatomic,strong) DrawerViewController *menuController;
@end
...
//}
//
- (DrawerViewController *)menuController
{
    if (!_menuController) {
        _menuController = [[DrawerViewController alloc] init];
    }
    return _menuController;
}
...
//            });
            // 更改
            dispatch_once(&onceToken, ^{
                navVC = [[HMHNavigationController alloc] initWithRootViewController:self.menuController.viewControllers[0]];
            });
            ...
            //关闭抽屉控制器
            [root closeDrawerAnimated:YES completion:nil];
