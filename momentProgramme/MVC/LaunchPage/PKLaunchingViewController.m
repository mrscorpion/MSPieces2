...
//    __weak typeof(self) weakSelf = self;
    [self.markImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        //约束相对坐标固定
        CGFloat viewHeight = self.view.bounds.size.height /4;
        make.centerX.equalTo(self.view.mas_centerX);
        make.centerY.mas_equalTo(self.view.mas_centerY).offset(-viewHeight);
    }];
...
//        [AllControllersTool createViewControllerWithIndex:0];
        //动画结束之后直接进入首页
        [AllControllersTool createViewControllerWithIndex:1];
    }];
...
- (UIImageView *)launchingBackgroundImageView
{
    if (!_launchingBackgroundImageView) {
        ...
        [_launchingBackgroundImageView setImage:[UIImage imageNamed:@"anjo5"]];
    }
...
- (UIImageView *)markImageView
{
    if (!_markImageView) {
        ...
        [_markImageView setImage:[UIImage imageNamed:@"PIECES"]];
